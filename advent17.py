...
#fix scaf
new_scaf = "#"
new_scaf += all_scafs[16]
all_scafs[16] = new_scaf
new_l = str(["." for _ in range(len(all_scafs[0])+2)])
new_line = ""
for ss in new_l:
  new_line += ss
new_scafs = [new_line]
...
while True:
  if all_scafs[robot_pos[0] + temp_dir[0]][robot_pos[1] + temp_dir[1]] == "#":
    ...
    if robot_pos in visited:
      intersections.append(robot_pos)
